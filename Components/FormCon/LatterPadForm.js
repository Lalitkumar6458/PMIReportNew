...
const LatterPadForm = ({ setTextChange, intialvalues, loading, handleChange, imageUrl, beforeUpload, formateId, isLaterPadEdit,setIsLaterPadCreated }) => {
  ...
  const onFinish = (values) => {
    ...
    if (isLaterPadEdit) {
      data['id'] = intialvalues.id
      axios.put(ApiEndPoint + "latterpad", data)
        .then((response) => {
      ...
      axios.post(ApiEndPoint + "latterpad", data)
        .then((response) => {
          // Handle the response from the backend
          console.log("response", response);
          if (response.status == 200) {
        ...
        <Form.Item>
          <div className="flex items-center justify-center gap-3 mt-4">
            ...
            <button
              ...
              type="sumbit"
            >
              {isLaterPadEdit ? "Update Latter Pad" : "Create Latter Pad"}
            </button>
          </div>
        </Form.Item>
